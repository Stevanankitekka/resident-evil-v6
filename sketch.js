...
function preload(){
  ...
  stevanImgfall = loadAnimation("assets/s4.png");
...
function setup() {
  createCanvas(windowWidth,windowHeight);
   ...
   stevan.addAnimation("StevanFall",stevanImgfall);
   stevan.scale = 0.3
   stevan.debug = true;
...
function draw(){
  ...
  textSize(20);
  fill("white");
  //ext(mouseX + ", " + mouseY, 200,100);
  text ("score:"+ score,1380,85);
 if (gameState === "PLAY"){
  ...
  if(keyWentDown("W")){
    ...
    for(var i = 0; i < zombieGroup.length; i++){
      if(zombieGroup.get(i).isTouching(stevan)){
        stevan.changeAnimation("StevanFall",stevanImgfall);
        gameState = "END";
        reset();
      }
    }
    ...
    for(var i = 0; i < zombieGroup.length; i++){
      if(zombieGroup.get(i).isTouching(deer2)){
        deer2.destroy();
        animalCount = animalCount-1;
      }
    }
